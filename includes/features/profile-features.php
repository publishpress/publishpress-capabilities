    ...
    <div class="wrap publishpress-caps-manage pressshack-admin-wrapper pp-capability-menus-wrapper profile-features <?php echo (empty($profile_features_elements) ? 'empty-elements' : ''); ?>">
        ...
        <form method="post" id="ppc-profile-features-form" action="admin.php?page=pp-capabilities-profile-features">
            ...
            <div class="pp-columns-wrapper pp-enable-sidebar clear">
                ...
                $banner_messages[] = '<div class="ppc-profile-features-info" style="padding: 8px; background-color: #e7f7fe; border-left: 4px solid #00a0d2; color: #0073aa; margin-bottom: 15px; ' . $info_display . '"><strong><span class="dashicons dashicons-info-outline" aria-hidden="true"></span></strong> ' . sprintf(esc_html__('When you refresh, we test as a user in the %s role and save profile elements.', 'capability-manager-enhanced'), '<strong>' . esc_html($current_role_name) . '</strong>') . '</div>';
